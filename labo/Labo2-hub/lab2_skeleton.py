...
def fully_connected_backward(W, b, X, output_grad):
    dX = output_grad @ W      # (n_samples, in_features)
    dW = output_grad.T @ X       # (out_features, in_features)
    db = np.sum(output_grad, axis=0)    # (out_features,)
...
def sigmoid_backward(X, output_grad):
    return (1 - sigmoid_forward(X)) * sigmoid_forward(X) * output_grad


def bce_forward(x, target):
    N = len(x)
    return np.sum(-target * np.log(x) - (1 - target) * np.log(1-x))/N


def bce_backward(x, target):
    N = len(x)
    return (-(target/x) + ((1-target)/(1-x)))/N
...
# ---------------------------- Training functions -----------------------------
def train(x_train, target_train, x_val=None, target_val=None, epoch_count=100, learning_rate=0.001):
    counts = [2, 25, 25, 1]
    ...
    for epoch in range(epoch_count):
        ...
        pred = sigmoid_forward(w)
        loss = bce_forward(pred, target_train)
        
        # Training: Backward pass
        grad = bce_backward(pred, target_train)
        grad = sigmoid_backward(pred, grad)
        grad, dW3, db3 = fully_connected_backward(W3, b3, h, grad)
        grad = relu_backward(v, grad)
        grad, dW2, db2 = fully_connected_backward(W2, b2, g, grad)
        grad = relu_backward(u, grad)
        grad, dW1, db1 = fully_connected_backward(W1, b1, x_train, grad)
        ...
        if x_val is not None and target_val is not None:
            ...
            pred_val = forward_pred(W1, b1, W2, b2, W3, b3, x_val)
            loss = bce_forward(pred_val, target_val)

            # Validation: Metrics
            losses_val.append(loss)        
            predicted_classes = (pred_val > 0.5).astype(int)
...
def forward_pred(W1, b1, W2, b2, W3, b3, X):
...
def show_decision_boundary(W1, b1, W2, b2, W3, b3):
    ...
    y = forward_pred(W1, b1, W2, b2, W3, b3, data)
...
def show_classification(W1, b1, W2, b2, W3, b3, X, title=''):
    ...
    # <Your code here, same as forward pass in train> 
    pred = forward_pred(W1, b1, W2, b2, W3, b3, X)
...
if mode == 'test':
    ...
    train(x_train, target_train, x_val, target_val, epoch_count=1000, learning_rate=0.01)

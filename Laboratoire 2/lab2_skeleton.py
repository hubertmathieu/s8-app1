...
def relu_backward(X, output_grad):
    derivatives = np.where(X < 0, 0, 1)
    return derivatives * output_grad

def sigmoid_forward(X):
    return 1 / (1 + np.exp(-X))

def sigmoid_backward(X, output_grad):
    return output_grad * sigmoid_forward(X) * (1 - sigmoid_forward(X))

def bce_forward(x, target):
    return np.sum(-target * np.log(x) - (1 - target) * np.log(1 - x))/len(x)

def bce_backward(x, target):
    return (-target/x + (1 - target) / (1 - x))/len(target)
...
def train(x_train, target_train, x_val=None, target_val=None, epoch_count=100, learning_rate=0.04):
    ...
    for epoch in range(epoch_count):
        ...
        grad_y = bce_backward(pred, target_train)
        grad_w = sigmoid_backward(pred, grad_y)
        grad_h, dw3, db3 = fully_connected_backward(W3, b3, h, grad_w)
        grad_v = relu_backward(v, grad_h)
        grad_g, dw2, db2 = fully_connected_backward(W2, b2, g, grad_v)
        grad_u = relu_backward(u, grad_g)
        _, dw1, db1 = fully_connected_backward(W1, b1, x_train, grad_u)
        ...
        if x_val is not None and target_val is not None:
            ...
            w = fully_connected_forward(W3, b3, h)
            pred = sigmoid_forward(w)
            loss = bce_forward(pred, target_val)
...
def show_decision_boundary(W1, b1, W2, b2, W3, b3):
    ...
    ax.imshow(1 - pred.reshape(x1.size, x2.size).T, cmap='bwr', extent=[-1, 1, -1, 1], vmin=0, vmax=1)
    fig.show()
...
if mode == 'test':
    ...
    train(x_train, target_train, x_val, target_val, epoch_count=500, learning_rate=0.01)
